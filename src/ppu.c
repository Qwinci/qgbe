...
void ppu_clock(Ppu* self) {
	...
	/*if (++self->cycle == SCANLINE_CYCLES) {
		...
		if (self->ly == 154) {
			self->ly = 0;
			ppu_set_mode(self, PPU_MODE_DRAW);
		}
	}
	if (self->cycle == 1 && self->ly == 144) {
		ppu_set_mode(self, PPU_MODE_V_BLANK);
		if (self->stat & STAT_IRQ_V_BLANK) {
			cpu_request_irq(&self->bus->cpu, IRQ_LCD_STAT);
		}
		cpu_request_irq(&self->bus->cpu, IRQ_VBLANK);
		self->frame_ready = true;
	...
	u16 tile_data_area = self->lcdc & 1 << 4 ? 0 : 0x1000;
	u16 tile_map_area = self->lcdc & 1 << 3 ? 0x1C00 : 0x1800;
	...
	for (u8 x = 0; x < LCD_WIDTH; ++x) {
		...
		u16 x_off = (self->scx / 8 + tile_x) & 0x1F;
		u16 y_off = 32 * (((self->ly + self->scy) / 8) & 0xFF);
		u8 tile = self->vram[tile_map_area + x_off + y_off];

		u8 tile_low;
		u8 tile_high;
		if (tile_data_area == 0x1000) {
			tile_low = self->vram[tile_data_area + fine_y * 2 + 16 * (i8) tile];
			tile_high = self->vram[tile_data_area + fine_y * 2 + 16 * (i8) tile + 1];
		}
		else {
			tile_low = self->vram[tile_data_area + fine_y * 2 + 16 * tile];
			tile_high = self->vram[tile_data_area + fine_y * 2 + 16 * tile + 1];
		}

		u8 color_id = (tile_low >> (7 - fine_x) & 1) | (tile_high >> (7 - fine_x) & 1) << 1;
		u8 color_index = self->bg_palette >> (2 * color_id) & 0b11;
		u32 color = PALETTE_COLORS[color_index];
		self->texture[self->ly * LCD_WIDTH + x] = color;
	}*/
}
